...
class InfiniteSampler(torch.utils.data.Sampler): # 无限采样器
    ...
    def __iter__(self):
        ...
        if self.shuffle:
            ...
            rnd.shuffle(order)
            window = int(np.rint(order.size * self.window_size)) # np.rint将数据转化为最接近的整数浮点型
            print(window)
        ...
        while True:
            ...
            if idx % self.num_replicas == self.rank:
                yield order[i]
            if window >= 2:
                j = (i - rnd.randint(window)) % order.size # -4 % 10 = 6
...
def check_ddp_consistency(module, ignore_regex=None):
    ...
    for name, tensor in named_params_and_buffers(module):
        ...
        assert (nan_to_num(tensor) == nan_to_num(other)).all(), fullname # tensor:本机变量, other:从主机0同步得到的变量，检查本机与主机数据是否有区别
...
def print_module_summary(module, inputs, max_nesting=3, skip_redundant=True):
    ...
    def post_hook(mod, _inputs, outputs):
        ...
        if nesting[0] <= max_nesting:
            outputs = list(outputs) if isinstance(outputs, (tuple, list)) else [outputs]
            outputs = [t for t in outputs if isinstance(t, torch.Tensor)]
            entries.append(Dnnlib.EasyDict(mod=mod, inputs=_inputs, outputs=outputs))
    hooks = [mod.register_forward_pre_hook(pre_hook) for mod in module.modules()]
    hooks += [mod.register_forward_hook(post_hook) for mod in module.modules()]
    ...
    for e in entries:
        ...
        param_size = sum(t.numel() for t in e.unique_params)
        buffer_size = sum(t.numel() for t in e.unique_buffers)
        input_shapes = [str(list(e.inputs[0].shape) if e.inputs[0] is not None else '-') for t in e.inputs]
        output_shapes = [str(list(e.outputs[0].shape)) for t in e.outputs]
        output_dtypes = [str(t.dtype).split('.')[-1] for t in e.outputs]
        rows += [[
            name + (':0' if len(e.outputs) >= 2 else ''),
