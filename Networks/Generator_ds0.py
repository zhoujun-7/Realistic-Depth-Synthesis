...
class StyleSynNet(torch.nn.Module):
    def __init__(self,
        ...
        **block_kwargs
        ...
        setattr(self, f'res{ri}To{ro}_0', block)
        self.unet_modules.append(f'res{ri}To{ro}_0')
    ...
    def forward(self, img, ws):
        ...
        # for name in self.unet_modules[-self.unet_scales:]:
        #     block = getattr(self, name)
        #     # features = torch.cat([features, temp_values[block.in_resolution]], dim=1)  ######### zj
        #     if hasattr(block, 'num_ws'):
        #         wx = ws.narrow(1, ws_idx, block.num_ws); ws_idx += block.num_ws
            #     features = block(features, wx)  ######### zj
            # else:
